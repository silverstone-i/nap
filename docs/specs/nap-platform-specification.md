...
Two declared variations exist, and neither is assembled by hand. An
admin-targeted router is produced by the same factory over a controller bound
to the admin handle; the route registry mounts it at the same path shape and
passes it the admin handle, and its operations run against the admin
repositories with no tenant transaction. An extension route may declare its
access as `anonymous`, which skips the authentication, active-tenant,
entitlement, and permission gates, or `authenticated`, which keeps the
authentication gate and skips the other three; every other step of the chain,
including tenant-input rejection, stays. Only the `admin-tenancy`
authentication router may declare either, because they exist so the routes that
...
deactivated, or demoted through the ordinary user routes.
[ADR 0004](../ADRs/0004-seeded-root-identity.md) records the decision.
...
| `ARCH-039`, `ARCH-040`                                     | Tests reject a second ordinary employee or client tenant membership, permit vendor multi-tenant membership, prove ordinary and privileged tenant-data requests resolve one active tenant and cell assignment, and prove the seeded root identity resolves the platform operator's tenant and cannot be locked, deactivated, or demoted through ordinary user routes |
