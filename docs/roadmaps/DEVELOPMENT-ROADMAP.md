...
| Cell registration and tenant provisioning UI           | Accepted | Verified — PR #23             | Product shell and navigation; Cell tenancy and provisioning; RBAC and module entitlement           |
| Operator bootstrap and Tenant Management consolidation | Accepted | Verified upon merge of PR #25 | Cell registration and tenant provisioning UI                                                       |
| Reference data and Core                                | Draft    | Not started                   | Operator bootstrap and Tenant Management consolidation; RBAC                                       |
...
**Design:** Accepted (owner approved, 2026-09-11). **Implementation:** Verified in [PR #23](https://github.com/silverstone-i/nap/pull/23), merged with passing required checks.
...
**Delivery context:** Historical Verified statuses and evidence for the control plane,
cell provisioning, RBAC, and product shell remain intact. They do not establish
that all prerequisites are available through the UI: the previously missing cell
registration UI prevented the complete operator workflow when the cell registry
was empty. This change supplies the Cells destination and closes that local UI gap.

**Accepted design:** Use the existing owners:
...
TEN-010 and SHELL-009 record the owner-approved UI behavior.
...
checks and evidence; verification became effective when PR #23 merged with required checks passing. Historical verification remains unchanged.

## Operator bootstrap and Tenant Management consolidation

Design: Accepted. Implementation: Verified upon merge of [PR #25](https://github.com/silverstone-i/nap/pull/25) with required checks passing. Depends on Cell registration and tenant provisioning UI. Required before Reference data and Core.

TEN-011, SHELL-010 and ADR 0015 own automatic greenfield operator bootstrap, dedicated management destinations and readiness reporting. [Delivery plan](../implementation-plans/operator-bootstrap-and-management.md) records validation. Existing PR #23 verification remains intact; its merge condition was satisfied on 2026-09-12 UTC with passing CI.
...
**Depends on:** Operator bootstrap and Tenant Management consolidation; RBAC and module
entitlement.
