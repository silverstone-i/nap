...
**Separate `system/` and `modules/` tiers.** Rejected. Schema scope and
licensability are recorded on the module descriptor, where they can be queried
and validated. A folder split adds nothing on top of that and hard-codes a
distinction the descriptor already carries.
