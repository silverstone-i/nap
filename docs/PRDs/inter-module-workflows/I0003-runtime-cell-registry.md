# I0003: Runtime Cell Registry And Routing
...
returns `503 CELL_UNAVAILABLE`. This PRD supplies it.
...
- Rechecking cells after startup, returning a recovered cell to service, and
  adding a cell without a restart. No PRD yet (see
...
- I0003-R001: Startup must read `CELL_DATABASES_<ENV>` as a JSON object whose keys are cell IDs. Local and test entries give an endpoint and use `NAP_APP_PSWD_<ENV>`; production entries give the endpoint and the `nap-app` password. A missing setting means no cells.
- I0003-R002: Malformed cell configuration (invalid JSON, a key that is not a UUID, or an entry with no valid endpoint) must stop startup with `INVALID_CONFIGURATION` naming the setting.
- I0003-R003: For each configured cell, startup must read its `admin.cells` record, connect as `nap-app`, confirm the connection works, and call `verifyPhysicalIdentity`. The cell is ready only if every step passes.
- I0003-R004: The registry must expose `readiness(cellId)`, returning `{ ready: true }` or `{ ready: false, reason }` with a reason from section 8. An ID the registry does not hold returns `CELL_NOT_CONFIGURED`.
- I0003-R005: The registry must expose `cellFor(session)`, which reads the session's tenant, finds its `cell_id` in `admin.tenants`, and returns that cell's connection when it is ready. Otherwise it throws `CELL_UNAVAILABLE`.
- I0003-R006: The API must pass the registry as the `runtime` collaborator to `selectTenant` and `getCellReadiness`.
- I0003-R007: Shutdown must close every cell connection, including those of cells that are not ready.

## 7. Business Rules And Invariants

- I0003-R008: A cell that fails to load must not stop startup or affect other cells. Admin routes and ready cells keep serving.
- I0003-R009: A cell that is not ready must not serve any tenant request. `cellFor` must never return its connection.
- I0003-R010: The browser never names a cell. `cellFor` takes the cell only from the session's tenant record (M0001-09-R006).
- I0003-R011: Startup must not run cell migrations (M0002-01-R010) and must not write to any cell or admin table.
...
A cell's state does not change after startup in this PRD. The cell
health and hot-add inter-module workflow (no PRD yet) adds rechecks.
...
- I0003-R012: Passwords and connection strings must not appear in the registry's readiness results, errors, logs, or API responses. Startup errors name only the setting.
- I0003-R013: A not-ready result carries only its reason code, not the compared values or the cell's endpoint.

## 13. Acceptance Criteria

| Criterion | Required result                                                                                                                                                                        | Requirements                                   |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| AC01      | Startup with a valid map loads each configured cell; an absent setting starts with no cells.                                                                                           | I0003-R001, I0003-R003                         |
| AC02      | Invalid JSON, a non-UUID key, or an entry with no endpoint stops startup with `INVALID_CONFIGURATION` and no credential in the output.                                                 | I0003-R002, I0003-R012                         |
| AC03      | Each row of the section 8 table produces its stated reason from `readiness`.                                                                                                           | I0003-R003, I0003-R004, I0003-R013             |
| AC04      | With one ready cell and one unreachable cell, selecting a tenant in the ready cell succeeds and one in the unreachable cell returns `503 CELL_UNAVAILABLE`; admin routes still answer. | I0003-R005, I0003-R006, I0003-R008, I0003-R009 |
| AC05      | `cell-readiness` reports the registry result for a ready and a not-ready cell.                                                                                                         | I0003-R006                                     |
| AC06      | `cellFor` returns a connection only for a ready cell resolved from the session's tenant record.                                                                                        | I0003-R005, I0003-R009, I0003-R010             |
| AC07      | Startup runs no migration and writes nothing; shutdown closes every cell connection.                                                                                                   | I0003-R007, I0003-R011                         |
