...
- **TEN-001 Registry.** Tenants retain pending/active/suspended status and gain
  ...
  Activation requires the configured registered cell. Active tenant assignment
...
- **TEN-003 Sessions.** Login with one eligible membership selects it; several
  ...
  requires one active tenant assigned to the configured cell. Restricted sessions
...
| `POST /admin-tenancy/v1/auth/select`       | Membership intent only; rejects another identity's binding and wrong-cell assignments                             |
