...
it('enforces real scoped assignments across companies, future projects, and a second cell', async () => {
  ...
  const a = await create(t.session, company, { code: 'A', name: 'Company A' });
  const b = await create(t.session, company, { code: 'B', name: 'Company B' });
...
it('refuses access changes when audit cannot persist and denies unavailable entitlement authority', async () => {
  ...
  await test.owner.none(
    'REVOKE SELECT ON admin.module_entitlements FROM $1:name',
    [test.fixture.role]
  );
  try {
    expect(
    ...
    await test.owner.none(
      'GRANT SELECT ON admin.module_entitlements TO $1:name',
      [test.fixture.role]
    );
  }
  expect(
