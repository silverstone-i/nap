...
/**
 * Does: Checks role credentials and creates missing setup or runtime roles.
 * Called by: local setup using nap_admin and Render setup using provider credentials.
 * Why: the specification assigns database and role creation to nap_admin and minimal runtime grants to nap_app.
 */
  ...
  await using(url, async db => {
    for (const [role, password] of [
      ...
      const row = await db.oneOrNone(
        `SELECT rolname, NOT rolcanlogin OR
   (rolname='nap_app' AND (rolsuper OR rolbypassrls OR rolcreatedb OR rolcreaterole OR rolreplication
   OR EXISTS(SELECT 1 FROM pg_auth_members WHERE member=r.oid))) AS unsafe FROM pg_roles r WHERE rolname=$1`,
    ...
    for (const [role, password] of [
      ...
      if (
        ...
        await db.none(
          role === 'nap_admin'
            ? 'CREATE ROLE $1:name LOGIN NOSUPERUSER NOBYPASSRLS CREATEDB CREATEROLE NOREPLICATION INHERIT PASSWORD $2'
