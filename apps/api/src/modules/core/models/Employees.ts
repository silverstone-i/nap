...
export class Employees extends TableModel<EmployeesRow> {
  ...
  async grantRuntime(role: string) {
    await this.db.none(
      'GRANT USAGE ON SCHEMA app,cell TO $1:name; GRANT SELECT,INSERT,UPDATE ON app.employees,app.clients,app.vendors,app.vendor_contacts,cell.tenants,cell.tenant_user_bindings TO $1:name',
      [role]
    );
  }
}
