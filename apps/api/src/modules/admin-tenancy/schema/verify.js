...
export async function verifyAdmin(handle, modules) {
  ...
  const schema = await db.one(
    `SELECT pg_get_userbyid(nspowner) AS owner, has_schema_privilege('nap-app',oid,'USAGE') AS usage,has_schema_privilege('nap-app',oid,'CREATE') AS create FROM pg_namespace WHERE nspname='admin'`
  );
  requireCondition(
  ...
  const tables = await db.any(
    `SELECT c.relname AS name,pg_get_userbyid(c.relowner) AS owner,c.relrowsecurity AS rls,c.relforcerowsecurity AS force FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='admin' AND c.relkind IN ('r','p') ORDER BY c.relname`
  );
  requireCondition(
  ...
  for (const table of tables) {
    for (const privilege of [
      ...
      const row = await db.one(
        ...
        ['nap-app', `admin.${table.name}`, privilege]
      );
      requireCondition(
        row.allowed ===
  ...
  const publicAccess = await db.one(
    `SELECT EXISTS(SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace CROSS JOIN LATERAL aclexplode(coalesce(c.relacl,acldefault('r',c.relowner))) a WHERE n.nspname='admin' AND a.grantee=0 UNION ALL SELECT 1 FROM pg_namespace n CROSS JOIN LATERAL aclexplode(coalesce(n.nspacl,acldefault('n',n.nspowner))) a WHERE n.nspname='admin' AND a.grantee=0) AS unsafe`
  );
  requireCondition(!publicAccess.unsafe, 'PUBLIC_ACCESS');
  const functions = await db.any(
    `SELECT p.proname AS name,p.prosrc AS body,p.prosecdef AS definer,pg_get_userbyid(p.proowner) AS owner, EXISTS(SELECT 1 FROM aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) a WHERE a.grantee=0) AS public FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='admin' ORDER BY p.proname`
  );
  requireCondition(
  ...
  const ledger = await db.any(
    'SELECT module_name,migration_id,hash FROM admin.schema_migrations ORDER BY module_name,migration_id'
  );
  ...
  try {
    await db.tx(async tx => {
      ...
      requireCondition(
        (await catalog(tx, 'admin')) === (await catalog(tx, scratch)),
