...
export default function ChangePasswordDialog({ open, onClose, onSuccess, forced = false }) {
  ...
  const ruleResults = useMemo(
    () => RULES.map((r) => ({ ...r, pass: r.test(newPassword) })),
    [newPassword],
  );
  const allRulesPass = ruleResults.every((r) => r.pass);
  const passwordsMatch = newPassword && newPassword === confirmPassword;
  const canSubmit =
    currentPassword && newPassword && confirmPassword && allRulesPass && passwordsMatch;
  ...
  return (
    ...
    >
      ...
      <DialogContent dividers>
        ...
        <Box component="form" id="change-password-form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <PasswordField
