...
export default function ApInvoicesPage() {
  ...
  return (
    <Box sx={pageContainerSx}>
      ...
      <ConfirmDialog open={archiveOpen} title="Archive Invoice" message={hasSelection ? (selectedRows.length === 1 ? `Archive invoice "${selectedRows[0].invoice_number}"?` : `Archive ${selectedRows.length} invoices?`) : ''} confirmLabel="Archive" confirmColor="error" loading={archiveMut.isPending} onConfirm={handleArchive} onCancel={() => setArchiveOpen(false)} />
      <ConfirmDialog open={restoreOpen} title="Restore Invoice" message={hasSelection ? (selectedRows.length === 1 ? `Restore invoice "${selectedRows[0].invoice_number}"?` : `Restore ${selectedRows.length} invoices?`) : ''} confirmLabel="Restore" confirmColor="success" loading={restoreMut.isPending} onConfirm={handleRestore} onCancel={() => setRestoreOpen(false)} />
