...
export default function ReceiptsPage() {
  ...
  const openEdit = useCallback(() => {
    if (!selected) return;
    setEditForm({
      receipt_date: selected.receipt_date?.slice(0, 10) ?? '', amount: selected.amount ?? '',
      method: selected.method ?? 'check', reference: selected.reference ?? '', notes: selected.notes ?? '',
    });
    setEditOpen(true);
  }, [selected]);
  ...
  const handleArchive = async () => {
    try { await archiveMut.mutateAsync({ id: selected.id }); toast('Receipt archived'); setArchiveOpen(false); setSelectionModel([]); } catch (err) { toast(errMsg(err), 'error'); }
  };
  const handleRestore = async () => {
    try { await restoreMut.mutateAsync({ id: selected.id }); toast('Receipt restored'); setRestoreOpen(false); setSelectionModel([]); } catch (err) { toast(errMsg(err), 'error'); }
  };
