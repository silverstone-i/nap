/**
 * @file Migration orchestrator — runs migrations for specified schemas with post-migration seeding
  ...
  try {
    for (const schemaName of schemaList) {
      ...
      try {
        ...
        console.warn(`Post-migration seeding failed for schema "${schemaName}":`, err?.message || err);
      }
    }
  } catch (error) {
