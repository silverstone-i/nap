...
const CATALOG_ENTRIES = [
  // ── Core ────────────────────────────────────────────────────────
  { module: 'core', router: null, action: null, label: 'Core Module', description: 'Entity management and RBAC configuration', sort_order: 100 },
  ...
  { module: 'core', router: 'sources', action: null, label: 'Sources', description: 'Polymorphic source registry', sort_order: 150, available_fields: ['table_id', 'source_type', 'label'] },
  { module: 'core', router: 'addresses', action: null, label: 'Addresses', description: 'Addresses linked to entities', sort_order: 160, available_fields: ['label', 'address_line_1', 'address_line_2', 'address_line_3', 'city', 'state_province', 'postal_code', 'country_code', 'is_primary'] },
  { module: 'core', router: 'phone-numbers', action: null, label: 'Phone Numbers', description: 'Phone numbers linked to entities', sort_order: 170, available_fields: ['phone_type', 'phone_number', 'is_primary'] },
  ...
  { module: 'core', router: 'policies', action: null, label: 'Policies', description: 'Permission grants per role', sort_order: 200, available_fields: ['role_id', 'module', 'router', 'action', 'level'] },
  { module: 'core', router: 'policy-catalog', action: null, label: 'Policy Catalog', description: 'Read-only permission discovery catalog', sort_order: 210 },
  ...
  { module: 'projects', router: 'project-clients', action: null, label: 'Project Clients', description: 'Client assignments per project', sort_order: 320, available_fields: ['project_id', 'client_id', 'role', 'is_primary'] },
  ...
  { module: 'projects', router: 'cost-items', action: null, label: 'Cost Items', description: 'Budget line items per task', sort_order: 370, available_fields: ['task_id', 'item_code', 'description', 'cost_class', 'cost_source', 'quantity', 'unit_cost'] },
  ...
  { module: 'projects', router: 'template-tasks', action: null, label: 'Template Tasks', description: 'Reusable task blueprints', sort_order: 400, available_fields: ['template_unit_id', 'task_code', 'name', 'duration_days', 'parent_code'] },
  { module: 'projects', router: 'template-cost-items', action: null, label: 'Template Cost Items', description: 'Reusable cost item blueprints', sort_order: 410, available_fields: ['template_task_id', 'item_code', 'description', 'cost_class', 'cost_source', 'quantity', 'unit_cost'] },
  { module: 'projects', router: 'template-change-orders', action: null, label: 'Template Change Orders', description: 'Reusable change order blueprints', sort_order: 420, available_fields: ['template_unit_id', 'co_number', 'title', 'reason', 'total_amount'] },
  ...
  { module: 'activities', router: 'deliverable-assignments', action: null, label: 'Deliverable Assignments', description: 'Employee assignments to deliverables', sort_order: 540, available_fields: ['deliverable_id', 'project_id', 'employee_id', 'notes'] },
  ...
  { module: 'bom', router: 'vendor-pricing', action: null, label: 'Vendor Pricing', description: 'Price history per vendor SKU', sort_order: 630, available_fields: ['vendor_sku_id', 'unit_price', 'unit', 'effective_date'] },
  ...
  { module: 'ap', router: 'ap-invoice-lines', action: null, label: 'AP Invoice Lines', description: 'Line items on vendor invoices', sort_order: 720, available_fields: ['invoice_id', 'description', 'amount', 'account_id', 'cost_line_id', 'activity_id'] },
  ...
  { module: 'ar', router: 'ar-invoice-lines', action: null, label: 'AR Invoice Lines', description: 'Line items on client invoices', sort_order: 820, available_fields: ['invoice_id', 'description', 'amount', 'account_id'] },
  ...
  { module: 'accounting', router: 'journal-entry-lines', action: null, label: 'Journal Entry Lines', description: 'Debit/credit lines per journal entry', sort_order: 930, available_fields: ['entry_id', 'account_id', 'debit', 'credit', 'memo', 'related_table', 'related_id'] },
  ...
  for (const entry of entries) {
    ...
    if (!existing) {
      await dbInstance.none(
        `INSERT INTO ${s}.policy_catalog (module, router, action, label, description, sort_order, valid_statuses, available_fields)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [entry.module, entry.router, entry.action, entry.label, entry.description, entry.sort_order, entry.valid_statuses || null, entry.available_fields || null],
      );
      inserted++;
    }
  }
