...
describe('Auth Controller Integration', () => {
  ...
  const res = await request(server).post('/api/core/v1/auth/login').send({});
    expect(res.status).toBe(400);
  });

  it('POST /auth/refresh - should return 401 without refresh token', async () => {
  const res = await request(server).post('/api/core/v1/auth/refresh');
    expect(res.status).toBe(401);
  });

  it('POST /auth/logout - should return 401 if not authenticated', async () => {
  const res = await request(server).post('/api/core/v1/auth/logout');
    expect(res.status).toBe(401);
  });

  it('GET /auth/check - should return 401 if no JWT is provided', async () => {
  const res = await request(server).get('/api/core/v1/auth/check');
    expect(res.status).toBe(401);
  });
  ...
  const login = await request(server).post('/api/core/v1/auth/login').send({
      email: 'testuser@example.com',
      password: 'TestPassword123',
    });
