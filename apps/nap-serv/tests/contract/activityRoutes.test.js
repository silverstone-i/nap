/**
 * @file Contract tests — activity module API shape verification
 * @module tests/contract/activityRoutes
 *
...
// Set env
process.env.ACCESS_TOKEN_SECRET = 'test-access-secret-activity';
process.env.REFRESH_TOKEN_SECRET = 'test-refresh-secret-activity';
...
const ACTIVITY_ENDPOINTS = [
  '/api/activities/v1/categories',
  '/api/activities/v1/activities',
  '/api/activities/v1/deliverables',
  '/api/activities/v1/deliverable-assignments',
  '/api/activities/v1/budgets',
  '/api/activities/v1/cost-lines',
  '/api/activities/v1/actual-costs',
  '/api/activities/v1/vendor-parts',
];

describe('Activity Routes Contract', () => {
  ...
  describe('POST create routes', () => {
    it('POST /api/activities/v1/categories creates a category', async () => {
      const cookies = await getAuthCookies();
      const res = await request
        .post('/api/activities/v1/categories')
        .set('Cookie', cookies)
        .set('x-tenant-code', 'NAP')
        .send({ tenant_id: 'tid', code: 'PLUMB', name: 'Plumbing', type: 'subcontract' });
      expect(res.status).toBe(201);
      expect(res.body.id).toBeDefined();
    });

    it('POST /api/activities/v1/categories rejects invalid type', async () => {
      const cookies = await getAuthCookies();
      const res = await request
        .post('/api/activities/v1/categories')
        .set('Cookie', cookies)
        .set('x-tenant-code', 'NAP')
        .send({ tenant_id: 'tid', code: 'BAD', name: 'Bad', type: 'invalid_type' });
      expect(res.status).toBe(400);
      expect(res.body.error).toContain('Invalid category type');
    });

    it('POST /api/activities/v1/cost-lines rejects invalid source_type', async () => {
      const cookies = await getAuthCookies();
      const res = await request
        .post('/api/activities/v1/cost-lines')
        .set('Cookie', cookies)
        .set('x-tenant-code', 'NAP')
        .send({ company_id: 'c1', deliverable_id: 'd1', activity_id: 'a1', source_type: 'invalid' });
      expect(res.status).toBe(400);
      expect(res.body.error).toContain('Invalid source_type');
    });

    it('POST /api/activities/v1/budgets creates a budget', async () => {
      const cookies = await getAuthCookies();
      const res = await request
        .post('/api/activities/v1/budgets')
        .set('Cookie', cookies)
        .set('x-tenant-code', 'NAP')
        .send({ deliverable_id: 'd1', activity_id: 'a1', budgeted_amount: '5000' });
      expect(res.status).toBe(201);
      expect(res.body.id).toBeDefined();
    });
  });
