...
1. List the ADRs and read the index:
   ...
   row, rows with no file, and rows whose Title/Status/Requirements disagree
   with the file.

2. Read each ADR that needs a row or a correction. Take the number and title
   from the H1 (`# NNNN — Title`) and the status from the `**Status:**` bullet,
   verbatim. Take the Requirements cell from the `**Requirements:**` bullet,
   verbatim; an ADR with no such bullet gets `—`.
...
4. Insert or update the row in ascending ADR-number order. For a superseded ADR,
   keep the row and set its status to `Superseded by ADR NNNN`, matching the
   supersession vocabulary in `docs/README.md`. For a deleted file, delete the
   row.
   Columns are ADR, Status, Requirements, Scope.
