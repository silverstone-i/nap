...
#
# Backs up the SRH tenant schema, rebuilds the entire database from
# scratch (admin + NapSoft root + SRH tenant), then restores SRH
# business data from the backup while keeping a freshly-seeded
# policy_catalog (fixes stale valid_statuses / available_fields).
#
# Prerequisites:
...
# 1c. Backup admin.nap_users rows for the SRH tenant (preserves password_hash)
"${PSQL[@]}" -c "\\COPY (
  SELECT u.*
...
# ══════════════════════════════════════════════════════════════════
echo "=== Step 3: Recreate public schema ==="
...
# ══════════════════════════════════════════════════════════════════
echo "=== Step 5: Provision SRH tenant ==="
cd "$SERV_DIR"
...
# ══════════════════════════════════════════════════════════════════
echo "=== Step 6: Restore SRH data from backup ==="

# 6a. Rewrite SQL dump: srh -> srh_restore
echo "  6a. Rewriting schema references for staging..."
perl -pe '
...
# 6b. Restore into staging schema
echo "  6b. Restoring into srh_restore..."
"${PSQL[@]}" -c "DROP SCHEMA IF EXISTS $STAGE_SCHEMA CASCADE;"
"${PSQL[@]}" -f "$BACKUP_DIR/srh_restore_schema_safe.sql"

# 6c. Remap tenant_id and tenant_code
echo "  6c. Remapping tenant_id and tenant_code..."
"${PSQL[@]}" <<'REMAP_SQL'
DO $$
DECLARE
...
# 6d. Generate FK-safe truncate statements (reverse dependency order)
echo "  6d. Generating FK-safe truncate order..."
...
# 6e. Generate FK-safe insert statements (dependency order)
#     Uses explicit column lists to skip generated columns (e.g. amount).
echo "  6e. Generating FK-safe insert order..."
...
# 6f. Execute truncate + insert in a transaction
echo "  6f. Loading data from srh_restore into srh..."
{
  echo "BEGIN;"
...
# ══════════════════════════════════════════════════════════════════
# Step 7: Restore admin.nap_users for SRH
# ══════════════════════════════════════════════════════════════════
echo "=== Step 7: Restore admin.nap_users for SRH ==="
...
# 7a. Try to restore from CSV backup (preserves password hashes)
...
# 7b. Create nap_users for any app-user employees still missing
...
echo "Archival dump:   srh_schema_${TIMESTAMP}.dump"
echo ""
